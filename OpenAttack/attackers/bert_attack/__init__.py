...
key_words = ['once','each','twice','every']
...
def get_ner_pos(star_index,enti,sentence):
    ent_len = len(enti.split(' '))
    if star_index==0:
        ner_index =  0
    else:
        front_sen = sentence[:star_index]
        if front_sen[-1]==' ':
            ner_index = len(front_sen.strip().split(' '))
        else:
            ner_index = len(front_sen.split(' '))-1
    return list(range(ner_index, ner_index + ent_len))
    #return list(set([ner_index,ner_index+ent_len-1]))
...
class BERTAttacker(ClassificationAttacker):
    ...
    def __init__(self, 
            ...
            threshold_pred_score : float = 0.7, #0.3改成0.6
            max_length : int = 512,
    ...
    def attack(self, victim: Classifier, input_: str, goal: ClassifierGoal):

        #TODO
        x_orig = input_.split('answer:')[0].lower()
        label = input_.split('answer:')[1]
        ...
        #get logic enti
        seq = feature.seq.replace('\n', '').lower()
        nlp = spacy.load("en_core_web_trf")
        doc = nlp(seq)
        ent_list = []
        for ent in doc.ents:
            ent_list += get_ner_pos(ent.start_char, ent.text, seq)
        ...
        orig_probs = torch.Tensor(victim.get_prob([feature.seq + ' answer: ' + label]))
        ...
        important_scores = self.get_important_scores(words, label,  victim, current_prob, goal.target, orig_probs)
        ...
        for top_index in list_of_index:
            ...
            # 如果有敏感实体，那么不替换
            if top_index[0] in ent_list or words[top_index[0]] in key_words:
                continue
            ...
            for substitute in substitutes:
                #如果要换的词在原句中出现过或者是关键词
                if substitute == tgt_word or substitute in words or substitute in key_words or len(re.findall(r"\d+\.?\d*", substitute))!=0:
                    continue  # filter out original word

                #反义词就不替换
                sy_sets = get_syn(tgt_word)
                sy_flag = 0
                for each_sy in sy_sets:
                    if substitute in each_sy or each_sy in substitute:
                        sy_flag = 1
                if sy_sets==1:
                    continue
                ...
                #TODO
                temp_prob = torch.Tensor(victim.get_prob([temp_text + ' answer: ' + label]))[0].squeeze()
                feature.query += 1
    ...
    def get_important_scores(self, words, words_label, tgt_model, orig_prob, orig_label, orig_probs):
        def process_sen(sen, la):
            sen = ' '.join(sen)
            sen = sen + ' answer: ' + la
            return sen
        ...
        #print(masked_words)
        #print(len(masked_words))
        #exit()
        texts = [process_sen(words, words_label) for words in masked_words]  # list of text of masked words
