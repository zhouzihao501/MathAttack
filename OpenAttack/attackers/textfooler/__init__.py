...
class TextFoolerAttacker(ClassificationAttacker):
    ...
    def attack(self, victim: Classifier, input_: str, goal: ClassifierGoal):
        ...
        * **x_orig** : Input sentence.
        ...
        #加入y
        orig_probs = victim.get_prob([x_orig+' answer: '+label])[0]
        ...
        def get_ner_pos(star_index, enti, sentence):
            ...
            ent_len = len(self.tokenizer.tokenize(enti))
            if star_index == 0:
                ner_index = 0
            else:
                front_sen = sentence[:star_index]
                if front_sen[-1] == ' ':
                    ner_index = len(self.tokenizer.tokenize(front_sen.strip()))
                else:
                    ner_index = len(self.tokenizer.tokenize(front_sen)) - 1
            return list(range(ner_index,ner_index+ent_len))
            #return list(set([ner_index, ner_index + ent_len - 1]))

        # get logic enti
        seq = input_.split('answer:')[0].lower()
        ...
        def process_sen(sen, la):
            sen = self.tokenizer.detokenize(sen)
            sen = sen + ' answer: ' + la
            return sen

        # # test
        #
        # pred_all = victim.get_pred([process_sen(sentence,label) for sentence in leave_1_texts][0:3])
        # for each_pred in pred_all:
        #     if goal.check(None, each_pred):
        #         print('yes')
        #         return self.tokenizer.detokenize(leave_1_texts[2])
        #     else:
        #         print('no')
        ...
        #加入y
        leave_1_probs = victim.get_prob([process_sen(sentence,label) for sentence in leave_1_texts])
        ...
        #     if word not in self.filter_words or word not in key_words or idx not in ent_list:
        #         print(word)
        # print("ok")
        # exit()
        ...
        synonym_words = [
            ...
            if word not in self.filter_words and word not in key_words and idx not in ent_list
            else []
            for idx, word, pos in words_perturb
        ]
        synonyms_all = []
        ...
        for idx, synonyms in synonyms_all:
            ...
            new_probs = victim.get_prob([process_sen(sentence,label) for sentence in new_texts])
            ...
            if np.sum(new_probs_mask) > 0:
                ...
                #add y
                pred = victim.get_pred([x_adv+' answer: '+label])[0]
