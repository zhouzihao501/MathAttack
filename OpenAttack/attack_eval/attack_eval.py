...
class AttackEval:
    ...
    def ieval(self, dataset : Iterable[Dict[str, Any]], num_workers : int = 0, chunk_size : Optional[int] = None) -> Generator[Dict[str, Any], None, None]:
        ...
        if num_workers > 0:
            ...
            # x_normal = []
            # for each in dataset['x']:
            #     x_normal.append(each.split('answer:')[0])
            # y_normal = dataset['y']
            # dataset_normal = datasets.Dataset.from_dict({'x':x_normal,'y':y_normal})
    ...
    def eval(self, dataset: Iterable[Dict[str, Any]], total_len : Optional[int] = None, visualize : bool = False, progress_bar : bool = False, num_workers : int = 0, chunk_size : Optional[int] = None):
        ...
        #     if res["success"]:
        #         store_case.append([res["data"]["x"],'success',res["result"]])
        #     else:
        #         store_case.append([res["data"]["x"], 'fail', ' '])
            ...
            if visualize and (TAG_Classification in self.victim.TAGS):
                ...
                if res["success"]:
                    ...
                    if Tag("get_prob", "victim") in self.victim.TAGS:
                        ...
                        try:
                            probs = self.victim.get_prob([x_orig+' answer: '+label, x_adv+' answer: '+label])
                        finally:
                        ...
                        self.victim.set_context(res["data"], None)
                        try:
                            preds = self.victim.get_pred([x_orig+' answer: '+label, x_adv+' answer: '+label])
                        finally:
                            self.victim.clear_context()
                        y_orig = int(preds[0])
                        y_adv = int(preds[1])
                    ...
                    if Tag("get_prob", "victim") in self.victim.TAGS:
                        ...
                        try:
                            #TODO
                            probs = self.victim.get_prob([x_orig+' answer: '+label])
                        finally:
                            self.victim.clear_context()
                        y_orig = probs[0]
                    elif Tag("get_pred", "victim") in self.victim.TAGS:
                        ...
                        try:
                            #TODO add label
                            preds = self.victim.get_pred([x_orig+' answer: '+label])
                        finally:
                            self.victim.clear_context()
                        y_orig = int(preds[0])
                    else:
